...
脚本使用稳定的 `declarationKey` 幂等维护三条全国电力交易气象任务声明。当前只启用 09:00 全国晨报；16:30 变化复核和 17:00 次日预报保留为禁用声明，避免日内重复播报，后续需要时可以显式恢复。晨报先筛查七大区域，再精查 6～10 个重点省份，将天气翻译为负荷、光伏、风电和净负荷的方向性影响；不预测电价或 MW。任务完成显式群发后以 `NO_REPLY` 收尾，且 cron 投递模式保持关闭，因此不会额外发送“已收到”等模型确认语。如果没有配置发送目标，任务会被创建为禁用状态且不投递，防止误发。旧的单目标变量 `WEATHER_SCHEDULE_FEISHU_TARGET` 仍可兼容读取，但新配置统一使用复数变量。
