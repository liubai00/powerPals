...
def weather_report_html(
    ...
    page_title = title or f"{region}气象数据工作台"
...
<head>
  ...
  <style>
    ...
    header {{ background: #0f766e; color: white; }}
    h1 {{ margin: 0 0 8px; }}
    header p {{ margin: 0; }}
    .report-scope {{ margin-top: 10px; color: rgba(255, 255, 255, 0.88); font-size: 14px; line-height: 1.6; }}
    .report-scope strong {{ color: #ffffff; }}
    .page-shell {{ width: min(1120px, calc(100% - 40px)); margin: 0 auto; }}
    header .page-shell {{ padding: 20px 0; }}
    main.page-shell {{ padding: 20px 0 32px; }}
    .toolbar {{ display: flex; flex-wrap: wrap; gap: 12px; align-items: center; margin: 16px 0; }}
    .button {{ display: inline-block; border: 0; background: #0f766e; color: white; text-decoration: none; padding: 10px 14px; border-radius: 6px; cursor: pointer; font: inherit; }}
    ...
    .chart-tooltip {{ position: fixed; z-index: 9999; display: none; max-width: 260px; pointer-events: none; padding: 8px 10px; border-radius: 6px; background: rgba(15, 23, 42, 0.92); color: #ffffff; font-size: 13px; line-height: 1.45; box-shadow: 0 8px 24px rgba(15, 23, 42, 0.2); }}
    .metric-tabs {{ display: flex; flex-wrap: wrap; gap: 8px; margin: 0 0 12px; }}
    .metric-tab {{ border: 1px solid #cbd5e1; background: #ffffff; color: #263445; border-radius: 6px; padding: 7px 12px; cursor: pointer; font: inherit; }}
    .metric-tab.is-active {{ border-color: #0f766e; background: #e6f4f1; color: #0f766e; font-weight: 600; }}
    .metric-chart {{ display: none; }}
    .metric-chart.is-active {{ display: block; }}
    .section-title {{ margin: 24px 0 10px; font-size: 17px; color: #172033; }}
    .table-wrap {{ width: 100%; overflow-x: auto; -webkit-overflow-scrolling: touch; }}
    .table-wrap.compact {{ max-height: 460px; overflow: auto; border: 1px solid #d8dee9; background: white; }}
    table {{ width: 100%; min-width: 760px; border-collapse: collapse; background: white; border: 1px solid #d8dee9; }}
    .table-wrap.compact table {{ border: 0; }}
    ...
    @media (max-width: 760px) {{ .page-shell {{ width: calc(100% - 24px); }} .chart-grid {{ grid-template-columns: 1fr; }} }}
  </style>
</head>
<body>
  <header>
...
def _line_chart_svg(
    ...
    left, right, top, bottom = 52, 18, (40 if second_points else 24), 42
    ...
    primary_series_label = primary_label or ("最高温" if second_points else "")
    secondary_series_label = second_label or ("最低温" if second_points else "")
    ...
    legend = (
        f'<line x1="{left + 86}" y1="18" x2="{left + 106}" y2="18" stroke="{color}" stroke-width="3" stroke-linecap="round" />'
        f'<text x="{left + 112}" y="22" font-size="12" fill="#475569">{html.escape(primary_series_label)}</text>'
        f'<line x1="{left + 182}" y1="18" x2="{left + 202}" y2="18" stroke="{second_color}" stroke-width="3" stroke-linecap="round" />'
        f'<text x="{left + 208}" y="22" font-size="12" fill="#475569">{html.escape(secondary_series_label)}</text>'
        if second_points
        else ""
    )
    return f"""<svg class="chart" viewBox="0 0 {width} {height}" role="img" aria-label="{html.escape(title)}">
  ...
  {second_line}
  {point_nodes(points, color, primary_series_label)}
  {point_nodes(second_points or [], second_color, secondary_series_label) if second_points else ""}
  {label_nodes}
</svg>"""
