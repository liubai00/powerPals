...
class FeishuClient:
    ...
    async def reply_interactive_card(self, message_id: str, card: dict[str, Any]) -> str:
        body = await self.reply_message(message_id, "interactive", card["card"])
        return body.get("data", {}).get("message_id", "")

    async def reply_text_message(self, message_id: str, text: str) -> str:
        body = await self.reply_message(message_id, "text", {"text": text})
        return body.get("data", {}).get("message_id", "")
    ...
    async def _post_message(
        ...
        for attempt in range(2):
            ...
            body = {"msg_type": msg_type, "content": json_dumps(content)}
            if json_extra:
