...
_PRICE_DIRECTION_REQUEST = re.compile(
    r"(?:电价|价格|现货|日前|实时).{0,12}(?:涨|跌|上涨|下跌|走高|走低|走强|走弱|转强|转弱|偏强|偏弱|上行|下行|利多|利空|利好|利淡|看涨|看跌|反弹|回升|冲高|下探|续涨|续跌|形成支撑|获得支撑|维持高位|维持低位|保持高位|保持低位|处于高位|处在高位|仍在高位|继续高位|保持强势|保持弱势|高位运行|低位运行|坚挺|疲软|抬升|回落|方向|承压)|"
    r"(?:涨|跌|上涨|下跌|走高|走低|走强|走弱|转强|转弱|偏强|偏弱|上行|下行|利多|利空|利好|利淡|看涨|看跌|反弹|回升|冲高|下探|续涨|续跌|维持高位|维持低位|高位运行|低位运行|坚挺|疲软|抬升|回落).{0,12}(?:电价|价格|现货)"
)
_BID_REQUEST = re.compile(
    r"报价|申报价|报多少|出价|竞价|(?:提高|降低|上调|下调)申报|申报(?:价格|电量|策略)|"
    r"(?:申报量|报量).{0,8}(?:往上调|往下调|增加|减少|加大|压减|上调|下调|抬高|压低|上浮|下浮)|"
    r"(?:增加|减少|加大|压减|上调|下调|抬高|压低|上浮|下浮).{0,4}(?:申报量|报量)|"
    r"电量.{0,6}(?:多|少)申报|(?:申报价|报价).{0,8}(?:抬高|压低|上调|下调|上浮|下浮)|"
    r"(?:申报|报)(?:高|低|贵|便宜)(?:一点|一些|点|些)?|(?:多报|少报)(?!告)(?:一点|一些|点)?|元/MWh|元每兆瓦时",
    re.IGNORECASE,
)
_POSITION_REQUEST = re.compile(
    r"仓位|持仓|头寸|敞口|做多|做空|看多|看空|偏多|偏空|"
    r"开多(?:单)?|开空(?:单)?|多单|空单|买入|卖出|增持|减持|"
    r"加仓|减仓|建仓|平仓|"
...
_UNSAFE_MARKET_CLAIM = re.compile(
    r"(?:建议|应该|应当|适合|可以).{0,8}(?:做多|做空|看多|看空|偏多|偏空|"
    r"开多(?:单)?|开空(?:单)?|买入|卖出|增持|减持|加仓|减仓|建仓|平仓)|"
    r"(?:仓位|持仓|头寸|敞口|多头|空头).{0,10}(?:可以|建议|应该|应当|适合|"
    r"增加|减少|加大|减小|加重|减轻|增持|减持|调整|操作)|"
    r"(?:电价|价格|现货).{0,12}(?:涨|跌|上涨|下跌|走高|走低|走强|走弱|转强|转弱|偏强|偏弱|上行|下行|利多|利空|利好|利淡|看涨|看跌|反弹|回升|冲高|下探|续涨|续跌|形成支撑|获得支撑|维持高位|维持低位|保持高位|保持低位|处于高位|处在高位|仍在高位|继续高位|保持强势|保持弱势|高位运行|低位运行|坚挺|疲软|抬升|回落|承压)|"
    r"(?:涨|跌|上涨|下跌|走高|走低|走强|走弱|转强|转弱|偏强|偏弱|上行|下行|利多|利空|利好|利淡|看涨|看跌|反弹|回升|冲高|下探|续涨|续跌|维持高位|维持低位|高位运行|低位运行|坚挺|疲软|抬升|回落).{0,8}(?:电价|价格|现货)|"
    r"(?:建议|应该|应当).{0,10}(?:提高|降低|上调|下调)?(?:报价|申报价|申报价格|出价)|"
    r"(?:建议|应该|应当|可以)?.{0,8}(?:(?:申报|报)(?:高|低)(?:一点|一些|点)?|(?:多报|少报)(?!告)(?:一点|一些|点)?)|"
    r"(?:申报量|报量).{0,8}(?:往上调|往下调|增加|减少|加大|压减|上调|下调|抬高|压低|上浮|下浮)|"
    r"(?:增加|减少|加大|压减|上调|下调|抬高|压低|上浮|下浮).{0,4}(?:申报量|报量)|"
    r"电量.{0,6}(?:多|少)申报|(?:申报价|报价).{0,8}(?:抬高|压低|上调|下调|上浮|下浮)|"
    r"(?:申报|报)(?:贵|便宜)(?:一点|一些|点|些)?|"
