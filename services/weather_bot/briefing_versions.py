...
def compare_market_risk_versions(
    ...
    if previous_run_id is None or not isinstance(previous_risks, list):
        return {
            "status": "unavailable",
            "reason": "previous_version_incomplete",
            "previous_run_id": previous_run_id,
        }
    previous_by_market = {
        str(item.get("market_id")): int(item.get("severity") or 0)
    ...
    upgraded = sum(
        current_by_market[market] > previous_by_market[market] for market in common_markets
    )
    downgraded = sum(
        current_by_market[market] < previous_by_market[market] for market in common_markets
    )
    unchanged = len(common_markets) - upgraded - downgraded
    return {
        "status": "available",
        "reason": "same_release_previous_day",
        "previous_run_id": previous_run_id,
        "previous_report_date": previous.get("report_date"),
        "comparable_markets": len(common_markets),
