...
WEATHER_BOT_ALIASES = ["AI气象预测小助手", "气象预测小助手", "气象小助手", "全国气象预测机器人", "云云"]
TASK_BOT_ALIASES = ["AI任务小助手", "任务小助手", "气象任务发布机器人", "点点"]
...
FORECAST_REPORT_CACHE_TTL_SECONDS = 600
...
def create_app(
    ...
    async def _handle_task_command(text: str, feishu_client: FeishuClient) -> dict[str, Any]:
        if _is_random_task_command(text):
            location = _random_task_location()
            task_request = WeatherTaskRequest(
    ...
    async def _handle_feishu_event(
        ...
        logger.warning(
            "feishu_event_text allowed_bot=%s event_keys=%s message_keys=%s mention_values=%s text=%r",
            allowed_bot,
        ...
        if allowed_bot in {FEISHU_WEATHER_BOT, FEISHU_TASK_BOT} and not _is_addressed_to_bot(
            text, event, allowed_bot, account
        ):
...
def _legacy_feishu_account(settings: Settings, verification_token_override: str | None = None) -> FeishuBotAccount:
    return FeishuBotAccount(
        ...
        mention_ids=_configured_mention_ids(None, settings.feishu_app_id),
    )
...
def _role_feishu_account(settings: Settings, role: str, legacy: FeishuBotAccount) -> FeishuBotAccount:
    if role == FEISHU_WEATHER_BOT:
        return FeishuBotAccount(
            ...
            aliases=_configured_bot_aliases(WEATHER_BOT_ALIASES, settings.feishu_weather_bot_aliases),
            mention_ids=_configured_mention_ids(
                settings.feishu_weather_bot_mention_ids,
                settings.feishu_weather_app_id or legacy.app_id,
            ),
        )
    if role == FEISHU_TASK_BOT:
        return FeishuBotAccount(
            ...
            aliases=_configured_bot_aliases(TASK_BOT_ALIASES, settings.feishu_task_bot_aliases),
            mention_ids=_configured_mention_ids(
                settings.feishu_task_bot_mention_ids,
                settings.feishu_task_app_id or legacy.app_id,
            ),
        )
    raise ValueError(f"Unknown Feishu bot role: {role}")


def _configured_bot_aliases(default_aliases: list[str], configured_aliases: str | None) -> tuple[str, ...]:
    return tuple(_unique_nonempty_values([*default_aliases, *_split_config_values(configured_aliases)]))


def _configured_mention_ids(configured_ids: str | None, app_id: str | None) -> tuple[str, ...]:
    values = _split_config_values(configured_ids)
    if app_id:
        values.append(app_id)
    return tuple(_unique_nonempty_values(values))


def _split_config_values(value: str | None) -> list[str]:
    if not value:
        return []
...
def _is_addressed_to_bot(
    ...
    event: dict[str, Any],
    allowed_bot: str,
    account: FeishuBotAccount | None = None,
) -> bool:
    aliases = _bot_aliases(allowed_bot, account)
    if not aliases:
        return True
    if _is_direct_chat(event):
        return True
    if account and _event_mentions_current_bot(event, account):
        return True
...
def _bot_aliases(allowed_bot: str, account: FeishuBotAccount | None = None) -> list[str]:
    if account and account.aliases:
        return list(account.aliases)
    if allowed_bot == FEISHU_WEATHER_BOT:
...
def _event_mentions_current_bot(event: dict[str, Any], account: FeishuBotAccount) -> bool:
    mention_ids = {item for item in account.mention_ids if item}
    if not mention_ids:
        return False
    ...
    for mention in mentions:
        mention_values = set(_string_values(mention))
        if mention_ids & mention_values:
            return True
    return False
...
async def _send_feishu_event_response(
    ...
    try:
        ...
        reply_to_message_id = _event_message_id(event) if _event_should_reply_to_message(event) else None
        if isinstance(card, dict):
            if reply_to_message_id:
                message_id = await feishu_client.reply_interactive_card(reply_to_message_id, card)
            else:
                message_id = await feishu_client.send_interactive_card(chat_id, card)
        elif isinstance(text, str) and text:
            if reply_to_message_id:
                message_id = await feishu_client.reply_text_message(reply_to_message_id, text)
            else:
...
def _event_should_reply_to_message(event: dict[str, Any]) -> bool:
    message = event.get("message", {})
    if not isinstance(message, dict):
        return False
    return any(message.get(field) for field in ("root_id", "parent_id", "thread_id", "topic_id"))
...
def _is_task_command(text: str) -> bool:
    return _is_random_task_command(text) or "气象任务" in text or ("气象" in text and "任务" in text)


def _is_random_task_command(text: str) -> bool:
    return "随机" in text and "任务" in text and any(keyword in text for keyword in ("发布", "创建", "发起", "生成"))


def _random_task_location() -> ResolvedLocation:
    locations_by_name = {location.name: location for location in BUILTIN_LOCATIONS.values()}
    return random.choice(sorted(locations_by_name.values(), key=lambda item: item.name))
