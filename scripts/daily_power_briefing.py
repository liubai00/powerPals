...
async def _fetch(service, market: str, city: str, start_date: str) -> dict[str, Any]:
    async with _SEM:
        try:
            request = m.ForecastRequest(region=city, target_date=start_date, days=2, granularity="1h")
            collected, errors = await m.collect_forecasts_with_errors(service, request)
            return {
                "market": market,
                "province": market,
                "city": city,
